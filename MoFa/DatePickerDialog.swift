...
class DatePickerDialog: UIView {
    ...
    fileprivate var datePickerMode: UIDatePicker.Mode!
    ...
    init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(DatePickerDialog.deviceOrientationDidChange(_:)), name: UIDevice.orientationDidChangeNotification, object: nil)
    }
    ...
    func show(_ title: String, datePickerMode: UIDatePicker.Mode = .dateAndTime, callback: @escaping ((_ date: Date) -> Void)) {
        show(title, doneButtonTitle: "Done", cancelButtonTitle: "Cancel", datePickerMode: datePickerMode, callback: callback)
    }
    
    func show(_ title: String, doneButtonTitle: String, cancelButtonTitle: String, defaultDate: Date = Date(), datePickerMode: UIDatePicker.Mode = .dateAndTime, callback: @escaping ((_ date: Date) -> Void)) {
        self.title = title
    ...
    fileprivate func addButtonsToView(_ container: UIView) {
        ...
        self.cancelButton.setTitle(self.cancelButtonTitle, for: UIControl.State())
        ...
        self.cancelButton.addTarget(self, action: #selector(DatePickerDialog.buttonTapped(_:)), for: UIControl.Event.touchUpInside)
        ...
        self.doneButton.setTitle(self.doneButtonTitle, for: UIControl.State())
        ...
        self.doneButton.addTarget(self, action: #selector(DatePickerDialog.buttonTapped(_:)), for: UIControl.Event.touchUpInside)
    ...
    func countScreenSize() -> CGSize {
        ...
        if interfaceOrientaion.isLandscape {
            let tmp = screenWidth
